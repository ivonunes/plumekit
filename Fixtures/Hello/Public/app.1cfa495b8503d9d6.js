function bootPlumeRuntime() {
  ...
  let syncNavigationHead = function(nextDocument) {
    ...
    let nextNames = new Set(nextMetas.map(function(node) { return node.getAttribute("name"); }));
    let currentMetas = Array.from(document.head.querySelectorAll("meta[name]"));
    let currentByName = new Map(currentMetas.map(function(node) { return [node.getAttribute("name"), node]; }));
    nextMetas.forEach(function(node) {
    ...
    currentMetas.forEach(function(node) {
