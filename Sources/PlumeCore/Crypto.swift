// Pure-Swift SHA-256 + HMAC, Embedded-clean ([UInt8], no Foundation/CryptoKit —
// swift-crypto is native-only and won't link in the guest). Used by CSRF token
// signing; deterministic and testable against known vectors.

public enum SHA256 {
    private static let k: [UInt32] = [
        0x428a2f98, 0x71374491, 0xb5c0fbcf, 0xe9b5dba5, 0x3956c25b, 0x59f111f1, 0x923f82a4, 0xab1c5ed5,
        0xd807aa98, 0x12835b01, 0x243185be, 0x550c7dc3, 0x72be5d74, 0x80deb1fe, 0x9bdc06a7, 0xc19bf174,
        0xe49b69c1, 0xefbe4786, 0x0fc19dc6, 0x240ca1cc, 0x2de92c6f, 0x4a7484aa, 0x5cb0a9dc, 0x76f988da,
        0x983e5152, 0xa831c66d, 0xb00327c8, 0xbf597fc7, 0xc6e00bf3, 0xd5a79147, 0x06ca6351, 0x14292967,
        0x27b70a85, 0x2e1b2138, 0x4d2c6dfc, 0x53380d13, 0x650a7354, 0x766a0abb, 0x81c2c92e, 0x92722c85,
        0xa2bfe8a1, 0xa81a664b, 0xc24b8b70, 0xc76c51a3, 0xd192e819, 0xd6990624, 0xf40e3585, 0x106aa070,
        0x19a4c116, 0x1e376c08, 0x2748774c, 0x34b0bcb5, 0x391c0cb3, 0x4ed8aa4a, 0x5b9cca4f, 0x682e6ff3,
        0x748f82ee, 0x78a5636f, 0x84c87814, 0x8cc70208, 0x90befffa, 0xa4506ceb, 0xbef9a3f7, 0xc67178f2,
    ]

    public static func hash(_ message: [UInt8]) -> [UInt8] {
        var h: [UInt32] = [
            0x6a09e667, 0xbb67ae85, 0x3c6ef372, 0xa54ff53a, 0x510e527f, 0x9b05688c, 0x1f83d9ab, 0x5be0cd19,
        ]

        // Pad: append 0x80, then zeros, then 64-bit big-endian bit length.
        var data = message
        let bitLength = UInt64(message.count) * 8
        data.append(0x80)
        while data.count % 64 != 56 { data.append(0) }
        for shift in stride(from: 56, through: 0, by: -8) {
            data.append(UInt8(truncatingIfNeeded: bitLength >> UInt64(shift)))
        }

        var w = [UInt32](repeating: 0, count: 64)
        var block = 0
        while block < data.count {
            for i in 0..<16 {
                let j = block + i * 4
                w[i] = UInt32(data[j]) << 24 | UInt32(data[j + 1]) << 16
                    | UInt32(data[j + 2]) << 8 | UInt32(data[j + 3])
            }
            for i in 16..<64 {
                let s0 = rotr(w[i - 15], 7) ^ rotr(w[i - 15], 18) ^ (w[i - 15] >> 3)
                let s1 = rotr(w[i - 2], 17) ^ rotr(w[i - 2], 19) ^ (w[i - 2] >> 10)
                w[i] = w[i - 16] &+ s0 &+ w[i - 7] &+ s1
            }

            var a = h[0], b = h[1], c = h[2], d = h[3]
            var e = h[4], f = h[5], g = h[6], hh = h[7]
            for i in 0..<64 {
                let s1 = rotr(e, 6) ^ rotr(e, 11) ^ rotr(e, 25)
                let ch = (e & f) ^ (~e & g)
                let t1 = hh &+ s1 &+ ch &+ k[i] &+ w[i]
                let s0 = rotr(a, 2) ^ rotr(a, 13) ^ rotr(a, 22)
                let maj = (a & b) ^ (a & c) ^ (b & c)
                let t2 = s0 &+ maj
                hh = g; g = f; f = e; e = d &+ t1
                d = c; c = b; b = a; a = t1 &+ t2
            }
            h[0] = h[0] &+ a; h[1] = h[1] &+ b; h[2] = h[2] &+ c; h[3] = h[3] &+ d
            h[4] = h[4] &+ e; h[5] = h[5] &+ f; h[6] = h[6] &+ g; h[7] = h[7] &+ hh
            block += 64
        }

        var out: [UInt8] = []
        for value in h {
            out.append(UInt8(truncatingIfNeeded: value >> 24))
            out.append(UInt8(truncatingIfNeeded: value >> 16))
            out.append(UInt8(truncatingIfNeeded: value >> 8))
            out.append(UInt8(truncatingIfNeeded: value))
        }
        return out
    }

    private static func rotr(_ x: UInt32, _ n: UInt32) -> UInt32 { (x >> n) | (x << (32 - n)) }
}

/// HMAC-SHA256 (RFC 2104).
public func hmacSHA256(key: [UInt8], message: [UInt8]) -> [UInt8] {
    var k = key
    if k.count > 64 { k = SHA256.hash(k) }
    if k.count < 64 { k += [UInt8](repeating: 0, count: 64 - k.count) }
    let ipad = k.map { $0 ^ 0x36 }
    let opad = k.map { $0 ^ 0x5c }
    let inner = SHA256.hash(ipad + message)
    return SHA256.hash(opad + inner)
}

/// Lowercase hex.
public func hexEncode(_ bytes: [UInt8]) -> String {
    let digits = Array("0123456789abcdef".utf8)
    var out: [UInt8] = []
    for b in bytes { out.append(digits[Int(b >> 4)]); out.append(digits[Int(b & 0x0F)]) }
    return decodeUTF8(out)
}

/// Constant-time byte comparison (no early exit on first mismatch).
public func constantTimeEqual(_ a: [UInt8], _ b: [UInt8]) -> Bool {
    if a.count != b.count { return false }
    var diff: UInt8 = 0
    for i in 0..<a.count { diff |= a[i] ^ b[i] }
    return diff == 0
}

/// PBKDF2-HMAC-SHA256 (RFC 2898). Derives `keyLength` bytes from `password`+`salt`
/// over `iterations` rounds. Built on the verified `hmacSHA256` so it's correct by
/// construction and Embedded-clean (byte ops only — no Foundation, no Unicode).
public func pbkdf2SHA256(password: [UInt8], salt: [UInt8], iterations: Int, keyLength: Int) -> [UInt8] {
    var output: [UInt8] = []
    var blockIndex: UInt32 = 1
    while output.count < keyLength {
        var message = salt
        message.append(UInt8((blockIndex >> 24) & 0xff))
        message.append(UInt8((blockIndex >> 16) & 0xff))
        message.append(UInt8((blockIndex >> 8) & 0xff))
        message.append(UInt8(blockIndex & 0xff))
        var u = hmacSHA256(key: password, message: message)
        var t = u
        var round = 1
        while round < iterations {
            u = hmacSHA256(key: password, message: u)
            for i in 0..<t.count { t[i] ^= u[i] }
            round += 1
        }
        output.append(contentsOf: t)
        blockIndex += 1
    }
    return Array(output.prefix(keyLength))
}

/// Decode lowercase/uppercase hex back to bytes; nil on odd length or a non-hex
/// digit. Byte-wise (Embedded-clean).
public func hexDecode(_ hex: [UInt8]) -> [UInt8]? {
    if hex.count % 2 != 0 { return nil }
    func nibble(_ c: UInt8) -> UInt8? {
        switch c {
        case 0x30...0x39: return c - 0x30          // 0-9
        case 0x61...0x66: return c - 0x61 + 10      // a-f
        case 0x41...0x46: return c - 0x41 + 10      // A-F
        default: return nil
        }
    }
    var out: [UInt8] = []
    var i = 0
    while i < hex.count {
        guard let hi = nibble(hex[i]), let lo = nibble(hex[i + 1]) else { return nil }
        out.append((hi << 4) | lo)
        i += 2
    }
    return out
}
