...
@Test func paginateDoesNotOverflowOnHugePageAndPer() async throws {
    ...
    for i in 0..<3 { let w = Widget2(name: "n\(i)"); try await w.save(in: db) }
    // (page-1)*per would trap without the saturating multiply; expect an empty page, no crash.
    let page = try await Widget2.all().paginate(page: 2_000_000, per: 2_000_000, in: db)
    #expect(page.items.isEmpty && page.hasMore == false)
    let ok = try await Widget2.all().order(by: Widget2.id).paginate(page: 1, per: 2, in: db)
