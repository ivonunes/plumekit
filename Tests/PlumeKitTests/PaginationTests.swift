...
@Suite struct PaginationTests {
    ...
    @Test func pageNumbersUrlsAndTotals() async throws {
        ...
        let page = try await PagedItem.all().order(by: PagedItem.id).paginate(page: 2, per: 20, withTotal: true, in: db)
    ...
    @Test func edgesHaveNoDanglingLinks() async throws {
        ...
        let first = try await PagedItem.all().order(by: PagedItem.id).paginate(page: 1, per: 20, in: db)
        ...
        let last = try await PagedItem.all().order(by: PagedItem.id).paginate(page: 2, per: 20, in: db)
...
extension PaginationTests {
    @Test func paginationURLsHandleExistingQueryString() async throws {
        ...
        let page = try await PagedItem.all().order(by: PagedItem.id).paginate(page: 1, per: 10, in: db)
    ...
    @Test func nonPositivePageSizeIsClamped() async throws {
        ...
        let page = try await PagedItem.all().order(by: PagedItem.id).paginate(page: 1, per: 0, in: db)
