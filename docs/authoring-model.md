...
The language stays small on purpose. Host applications provide the project shape, load templates, pass data, resolve assets, and decide how collected resources are emitted.
...
When markup needs supporting CSS or behaviour, colocate the resource with the markup that owns it:
...
- Use UpperCamelCase for component names, such as `PostCard` or `SiteHeader`.
...
- Loading templates and component files.
- Passing context data.
- Marking trusted HTML as `PlumeSafeHTML`.
- Providing functions such as `asset()`.
- Resolving `@style(file:)`, `@script(file:)`, and `@image`.
- Emitting collected resources.
- Including the runtime when `renderResult.requiresRuntime` is true.

This boundary is why the same Plume syntax can work inside Inkstead Writer and inside another Swift application with different asset and deployment rules.

## Common Mistakes

- Do not write Liquid syntax like `{{ title }}` or `{% if post %}`.
- Do not use `| raw` for user-provided or unsanitized strings.
