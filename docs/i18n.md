...
PlumeKit resolves a request's language once and makes a `t()` function available to handlers and views, so you never pass a locale around. Strings are compiled into the app, with no runtime file parsing, so the same translations work natively and on the edge.
...
The build compiles them into a `plumeKitTranslations` value automatically.

Set the fallback language, used when a request matches none, in `plumekit.toml`:
...
A missing key renders the key itself, so an untranslated string is visible rather than blank. A missing placeholder is left in place.
...
### Honouring a saved preference

A signed-in user may have a saved language that should win over the browser's headers. Override the active language for the rest of the request with `useLocale`:
